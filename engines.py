...
class AlpacaTrader():
    def __init__(self):
        self.auth_client_alpaca = tradeapi.REST(Config.get('Alpaca', 'api_key'), Config.get(
            'Alpaca', 'api_secret'), APCA_API_BASE_URL, api_version='v2')  # or use ENV Vars shown below
        self.account = self.auth_client_alpaca.get_account()
        self.equity = float(self.account.equity)

    def buy_long(self, asset):
        try:
            self.auth_client_alpaca.submit_order(
                symbol=asset, qty=10, side='buy', type='market', time_in_force='gtc')
    ...
    def sell_short(self, asset):
        try:
            self.auth_client_alpaca.submit_order(
                symbol=asset, qty=10, side='sell', type='market', time_in_force='gtc')
        except Exception as e:
            print(str(e))
        finally:
...
class BitmexTrader():
        ...
        if self.trade:
            ...
            if self.short:
                close = self.auth_client_bitmex.Order.Order_new(
                    symbol='XBTUSD', ordType='Limit', execInst='Close', price=pric - 5, timeInForce='FillOrKill').result()
                time.sleep(1)
                runs = 1
                while close[0]['ordStatus'] != 'Filled':
                    close = self.auth_client_bitmex.Order.Order_new(
                        symbol='XBTUSD', ordType='Limit', execInst='Close', price=pric - (5 - runs * 0.5), timeInForce='FillOrKill').result()
                    runs += 1
                    time.sleep(1)
                self.short = False
            ...
            try:
                ...
                order = self.auth_client_bitmex.Order.Order_new(
                    symbol='XBTUSD', orderQty=order_q, price=pric - 3, timeInForce='FillOrKill').result()
                time.sleep(1)
                runs = 1
                while order[0]['ordStatus'] != 'Filled':
                    order = self.auth_client_bitmex.Order.Order_new(
                        symbol='XBTUSD', orderQty=order_q, price=pric - (runs * 0.5), timeInForce='FillOrKill').result()
                    runs += 1
                    time.sleep(1)
            except HTTPServiceUnavailable as e:
                self.client.chat_postMessage(
                    channel=self.channel_trades, text='error: ' + str(e) + ' retrying...')
                ord = ''
                while ord != 'Filled':
                    time.sleep(0.6)
                    order = self.auth_client_bitmex.Order.Order_new(
                ...
                try:
                    order = self.auth_client_bitmex.Order.Order_new(
                        symbol='XBTUSD', orderQty=order_q - 10).result()
                except:
                    self.client.chat_postMessage(
                        channel=self.channel_trades, text='error: ' + str(r) + ' FATAL!!! order not placed')
            finally:
                self.slips.append(
                    float(abs(ind - float(order[0]['price'])) / 0.5))
                print('bought long on bitmex: ' + str(order[0]['orderQty']) + ' @ ' + str(order[0]['price']), 'slip: $' + str(abs(ind - float(
                    order[0]['price']))), 'ticks: ' + str((ind - float(order[0]['price'])) / 0.5), 'average tick slip: ' + str(mean(self.slips)))
            try:
            ...
            try:
                ...
                self.auth_client_bitmex.Order.Order_new(symbol='XBTUSD', ordType='Stop', stopPx=floor(
                    (price - (price * self.stop_loss / self.leverage)) * 0.5) / 0.5, orderQty=-order_q).result()
            self.client.chat_postMessage(channel=self.channel_trades, text='bought: ' + str(round(float(
                order[0]['orderQty']) / self.leverage, 3)) + ' XBT with ' + str(self.leverage) + ' X leverage at $' + str(order[0]['price']))
            self.long = True
    ...
    def sell_short(self, ex, pair, ind, pric):
        ...
        if self.trade:
            ...
            if self.long:
                close = self.auth_client_bitmex.Order.Order_new(
                    symbol='XBTUSD', ordType='Limit', execInst='Close', price=pric + 5).result()
                time.sleep(1)
                runs = 1
                while close[0]['ordStatus'] != 'Filled':
                    close = self.auth_client_bitmex.Order.Order_new(
            ...
            '''
            try:
                order = self.auth_client_bitmex.Order.Order_new(symbol='XBTUSD', orderQty=-floor(
                    bal * self.leverage * price) + 10, price=pric + 3, timeInForce='FillOrKill').result()
                time.sleep(1)
                runs = 1
                while order[0]['ordStatus'] != 'Filled':
                    order = self.auth_client_bitmex.Order.Order_new(symbol='XBTUSD', orderQty=-floor(
                        bal * self.leverage * price) + 10, price=pric - (runs * 0.5), timeInForce='FillOrKill').result()
                    runs += 1
                    time.sleep(1)
            except HTTPServiceUnavailable as e:
                ...
                while ord != 'Filled':
                    time.sleep(0.6)
                    order = self.auth_client_bitmex.Order.Order_new(
                        symbol='XBTUSD', orderQty=-floor(bal * self.leverage * price) + 10).result()
                    ord = order[0]['ordStatus']
            except HTTPBadRequest as r:
                ...
                self.slips.append(
                    float(abs(ind - float(order[0]['price'])) / 0.5))
                print('sold short on bitmex: ' + str(order[0]['orderQty']) + ' @ ' + str(order[0]['price']), 'slip: $' + str(abs(ind - float(
                    order[0]['price']))), 'ticks: ' + str((ind - float(order[0]['price'])) / 0.5), 'average tick slip: ' + str(mean(self.slips)))

            try:
                self.auth_client_bitmex.Order.Order_new(symbol='XBTUSD', ordType='MarketIfTouched', stopPx=floor(
                    price * (1 - self.take_profit / self.leverage) * 0.5) / 0.5, orderQty=floor(bal * self.leverage * price) + 10).result()
            except HTTPServiceUnavailable:
                print('503 retrying...')
                time.sleep(0.6)
                self.auth_client_bitmex.Order.Order_new(symbol='XBTUSD', ordType='MarketIfTouched', stopPx=floor(
            ...
            try:
                self.auth_client_bitmex.Order.Order_new(symbol='XBTUSD', ordType='Stop', stopPx=floor(
                    (price + (price * self.stop_loss / self.leverage)) * 0.5) / 0.5, orderQty=floor(bal * self.leverage * price) + 10).result()
            except HTTPServiceUnavailable:
                print('503 retrying...')
                time.sleep(0.6)
                self.auth_client_bitmex.Order.Order_new(symbol='XBTUSD', ordType='Stop', stopPx=floor(
            ...
            self.client.chat_postMessage(channel=self.channel_trades, text='shorted: ' + str(round(
                float(-order[0]['orderQty']), 3) / self.leverage) + ' XBT with ' + str(self.leverage) + ' X leverage at $' + str(order[0]['price']))
            self.short = True
