...
def load_dfs(asset, files):
    ...
    if 1 == 1 or not glob.glob('../../loaded'+frm+too+'.csv'):
        ...
        a = pd.read_csv('../../loaded'+frm+too+'.csv', header=None, low_memory=False, dtype={2: float}, usecols=[0, 2])
...
def load_dfs_mult(asset, files):
    ...
    if 1==1 or not glob.glob('../../loaded'+frm+too+'.csv'):
        ...
        combined = pd.read_csv('../../loaded'+frm+too+'.csv', header=None, low_memory=False, dtype={2: float}, usecols=[0, 2])
